        ...
        <Section title="Certificados" icon={Cpu}>
            <div className="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-3">
                {certifications.map((cert, index) => {
                    ...
                    const title = hasProvider ? cert.split(':').slice(1).join(':').trim() : cert;

                    return (
                        <div key={index} className="bg-slate-50 pl-4 py-3 pr-3 rounded-r border-l-[3px] border-slate-300 break-inside-avoid flex flex-col justify-center">
                            {hasProvider && (
